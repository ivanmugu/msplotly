...
Multiple Sequence Plotter with Plotly (MSPlotly) uses GenBank files (.gb) to align the
sequences and plot the genes. MSPlotly uses the information from the `CDS` features
section to plot the genes. To customize the colors for plotting genes, you can add a
`Color` tag in the `CDS` features with a color in hexadecimal. For example, add the tag
`/Color="#00ff00"` to show a green gene. To reduce the manual manipulation of the GenBank
file, you can edit the file with `Geneious` or another software and export the file with
the new annotations.
...
def adjust_positions_sequences_right(
    ...
    """Adjust the position of the sequences to the right including CDSs."""
    for record in gb_records:
...
def plot_polygon(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            fillcolor=color,
            name=name,
            customdata=custom_data,  # Use this parameter to store extra data like homology
...
def annotate_genes_top_using_trace_customdata(figure: Figure) -> Figure:
    for trace in figure["data"]:
        if ("name" in trace) and ("gene_info" in trace["customdata"][0]):
            gb_record = int(trace["customdata"][1].split("=")[1])
            if gb_record != 0:
                continue
            print(trace)
            x_start = int(trace["customdata"][3].split("=")[1])
            x_end = int(trace["customdata"][4].split("=")[1])
            y = int(trace["customdata"][5].split("=")[1])
...
def change_homoloy_color_traces(
    ...
    This function gets the identity (homology) value from the trace's customdata.
    """
    # Get new colorscale
    ...
    for trace in figure["data"]:
        if ("name" in trace) and ("Identity:" in trace["name"]):
            # Get identity information from customdata
            identity = trace["customdata"][0]
...
def make_alignments_for_dash(
    user_input: UserInput,
) -> tuple[BlastnAlignment, GenBankRecord]:
    """BLAST nucleotide sequences to make alignments."""
    # Create fasta files for BLASTing using the gb files
    faa_files = msp.make_fasta_files(user_input.input_files, user_input.output_folder)
    # Run blastn locally to make alignments.
    blast_xml_results = msp.run_blastn(faa_files, user_input.output_folder)
    ...
    # results.
    alignments = msp.get_alignment_records(blast_xml_results)
    # Delete the xml documents.
    msp.delete_files(blast_xml_results)
    # Make a list of `GenBankRecord` classes using the gb files.
    gb_records = msp.get_gb_records(user_input.input_files)
...
#                                  MAIN FUNCTION GUI                                     #
# ====================================================================================== #
def make_figure_gui(user_input: UserInput) -> tuple[Figure, float, float]:
    ...
    if user_input.alignments_position == "center":
    ...
    colorscale = get_truncated_colorscale(
        colorscale_name=user_input.identity_color,
        vmin=user_input.colorscale_vmin,
        vmax=user_input.colorscale_vmax,
    )
    ...
    straight_homology_regions = (
        True if user_input.straight_homology_regions == "straight" else False
    )
    fig = plot_homology_regions(
        ...
        minimum_homology_length=user_input.minimum_homology_length,
    ...
    # Plot the genes
    fig = plot_genes(fig, gb_records, name_from=user_input.annotate_genes_with)
    # Annotate genes
    if user_input.annotate_genes == "top":
        fig = annotate_genes_top_using_trace_customdata(fig)
    if user_input.annotate_genes == "bottom":
        fig = annotate_genes_bottom_using_trace_customdata(fig)
    if user_input.annotate_genes == "top-bottom":
        fig = annotate_genes_top_using_trace_customdata(fig)
        fig = annotate_genes_bottom_using_trace_customdata(fig)
    # Annotate DNA sequences
    if (sequence_name := user_input.annotate_sequences) != "no":
    ...
    # Plot DNA scale
    # if user_input.add_scale_bar:
    #     fig = plot_scale(fig, longest_sequence)
    fig = plot_scale(fig, longest_sequence, user_input.add_scale_bar)
