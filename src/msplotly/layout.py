...
TMP_PATH = Path("/Users/msp/Documents/Coding/python_projects/MSPlotly/tmp")
SEQUENTIAL_COLOR_SCALES = [
    name for name in dir(px.colors.sequential) if not name.startswith("_")
]

# TODO: Make a function to create tmp directory
        ...
        children=[
            dbc.Row(  # = UPLOAD FILES SECTION = #
                [
                    ...
                    html.Div(
                        [  # Div to center AgGrid
                            # Table to display file names and paths
                            dag.AgGrid(
                                id="files-table",
                                columnDefs=[
                                    {
...
def update_file_table(filenames, contents, n_clicks, current_row_data, selected_rows):
...
# TODO: Don't forget to implement this function to clean tmp folder
# Clean up the temporary file after BLAST is done using pathlib.Path
def clean_up_temp_file(temp_file_path):
    try:
        file_path = Path(temp_file_path)  # Convert to a Path object
...
# ====================================================================================== #
#                           Functions to plot the alignment                              #
# ====================================================================================== #
...
def main_plot(
    ...
    if (button_id == "draw-button") and virtual:
        ...
        USER_INPUT.set_colorscale_to_extreme_homologies = is_set_to_extreme_homologies
    ...
    if button_id == "change-homology-color-button":
        ...
        return (fig, selected_trace_store_state, None, extreme_homologies_values_state)
    ...
    if (active_tab == "tab-edit") and (click_data is not None) and select_button_state:
...
def download_plot(
    ...
    return dict(base64=True, content=encoded, filename=figure_name, type=figure_format)
