...
class MakeFigure:
    ...
    def make_colormap(
        ...
        if min_val == 0 and max_val == 1:
            ...
            new_cmap = colors.LinearSegmentedColormap.from_list(name, truncated_cmap)
            return new_cmap
    ...
    def draw_colorbar(self, fig, ax: Axes) -> None:
        ...
        print(
            ...
            self.lowest_homology,
            self.highest_homology,
        )
        ...
        if self.lowest_homology != self.highest_homology:
            boundaries = np.linspace(self.lowest_homology, self.highest_homology, 100)
            # ax.set_position([0, 0, 0.9, 1])
            axins = inset_axes(
                ax,
                ...
                loc="lower right",
                bbox_to_anchor=(0.0, 0.04, 0.95, 0.95),
                bbox_transform=ax.transAxes,
                borderpad=0,
            )
            colormap = fig.colorbar(
    ...
    def plot_genes(self, ax: Axes) -> None:
        ...
        y_distance = len(self.gb_records) * self.y_separation
        arrowstyle = mpatches.ArrowStyle("simple", head_width=0.5, head_length=0.2)
    ...
    def plot_arrows(self, ax: Axes) -> None:
        ...
        for gb_record in self.gb_records:
            for gene in gb_record.cds:
                arrow = Arrow(
                    x1=gene.start, x2=gene.end, y=y_distance, head_height=head_height
                )
    ...
    def determine_figure_size(self, num_alignments: int) -> tuple[float, float]:
    ...
    def save_plot(self) -> None:
        """Save plot."""
        plt.savefig(fname=self.figure_name, format=self.figure_format, dpi=self.dpi)
